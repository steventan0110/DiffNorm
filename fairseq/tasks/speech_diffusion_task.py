...
from fairseq.data.audio.repr_to_vae_dataset import ReprToVAEDatasetCreator
...
@register_task("speech_diffusion")
class SpeechDiffusionTask(LegacyFairseqTask):
    @classmethod
    def add_args(cls, parser):
        ...
        parser.add_argument(
            "--mu",
            type=float,
            help="rescale factor for latent repr from VAE"
        )
    ...
    def __init__(self, args, tgt_dict):
        ...
        # TODO: make T, beta params from argument instead of hard coded
        # self.diff_params = self.calc_diff_params(50, 0.0001, 0.05)
        # following AudioLDM papear, we use T=1000, beta_0=0.0015, beta_T=0.0195
        self.diff_params = self.calc_diff_params(1000, 0.0015, 0.0195)
        self.mu = args.mu
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        self.datasets[split] = ReprToVAEDatasetCreator.from_tsv(
            src_feat_dir=self.args.src_feat_dir,
            vae_feat_dir=self.args.vae_feat_dir,
    ...
    @staticmethod
    def calc_diff_params(T, beta_0, beta_T, beta=None, fast=False):
        """
        Copied from Sashimi: https://github.com/albertfgu/diffwave-sashimi/blob/master/utils.py
        ...
        _dh = {}
        _dh["T"], _dh["Beta"], _dh["Alpha"], _dh["Alpha_bar"], _dh[
            "Sigma"] = T, Beta, Alpha, Alpha_bar, Sigma
        return _dh
