...
class SpeechToSpeechTask(LegacyFairseqTask):
    ...
    def add_args(cls, parser):
        ...
        parser.add_argument(
            "--use_side",
            action="store_true",
            help="set if using non-uniform dist when giving masks",
        )
    ...
    def __init__(self, args, tgt_dict, infer_tgt_lang_id=None):
        ...
        print("############################################################")
        if not hasattr(args, "use_side"):
            args.use_side = False
        print("use non-uniform masking: ", args.use_side)
    ...
    def setup_task(cls, args, **kwargs):
        if "/scr-ssd/chenyuz" in args.data:
            args.data = args.data.replace("/scr-ssd/chenyuz", "/weka/scratch/jzhan237/diff_s2s")
