...
class IterativeRefinementGenerator(object):
    def __init__(
        ...
        self.models = models
        self.use_true_length = use_true_length
    ...
    def generate(self, models, sample, prefix_tokens=None, constraints=None):
        ...
        if self.use_true_length:
            ...
            target_length = target.ne(self.pad).sum(dim=1)
            assert self.beam_size == 1, "beam search is not supported with true length"


        # initialize
        encoder_out = model.forward_encoder([src_tokens, src_lengths])
        prev_decoder_out = model.initialize_output_tokens(encoder_out, src_tokens, target_length)
        ...
        max_seq_len = prev_output_tokens.shape[1]
        ...
        for step in range(self.max_iter + 1):
            ...
            # if max_seq_len > 70:
            #     print(decoder_out.output_tokens[0, :])
            # import ipdb
            # ipdb.set_trace()
            if self.adaptive:
