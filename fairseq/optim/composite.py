...
from fairseq.optim.lr_scheduler.reduce_lr_on_plateau import ReduceLROnPlateauLRScheduleConfig
...
class FairseqCompositeOptimizer(FairseqOptimizer):
    ...
    def __init__(self, cfg: CompositeOptimizerConfig, params):
        # when this optimizer is used, we are doing GAN-like training, hard code the config here since command line tool is not supported

        cfg = OmegaConf.to_container(cfg, resolve=True)
        generator_config = OptimizerAndSchedulerConfig(
            lr=[0.0005],
            ...
            lr_scheduler=ReduceLROnPlateauLRScheduleConfig(
        ...
        discriminator_config = OptimizerAndSchedulerConfig(
            lr=[0.0001],
            optimizer=FairseqAdamConfig(
                _name="adam",
                adam_betas=[0.9, 0.98],
            ...
            lr_scheduler=ReduceLROnPlateauLRScheduleConfig(
                _name="reduce_lr_on_plateau",
                warmup_updates=4000, # cause the first 10k step it will be called
                warmup_init_lr=0.000001,
                lr_patience=5,
                lr_shrink=0.99,
                maximize_best_checkpoint_metric=False
        ...
        cfg["groups"]["generator"] = generator_config
        cfg["groups"]["discriminator"] = discriminator_config
        # print(cfg)
