...
def get_waveform(
    ...
    if isinstance(path_or_fp, str):
        ...
        if ext not in SF_AUDIO_FILE_EXTENSIONS and ext != ".mp3":
    ...
    if ext == ".mp3":
        import librosa
        # for our cm4 downloaded mp3 file, we need to process them
        waveform, sample_rate = librosa.load(path_or_fp, sr=output_sample_rate)
...
def get_features_from_npy_or_audio(path, waveform_transforms=None):
    ...
    if ext not in FEATURE_OR_SF_AUDIO_FILE_EXTENSIONS and ext != ".mp3":
