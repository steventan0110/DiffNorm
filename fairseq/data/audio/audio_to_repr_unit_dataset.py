...
@dataclass
class AudioToReprDatasetItem(object):
    index: int
...
class AudioToReprUnitDataset(FairseqDataset):
    def __init__(self,
                 split: str,
                 is_train_split: bool,
                 cfg: S2SDataConfig,
    ...
    def __getitem__(self, index: int) -> AudioToReprDatasetItem:
        ...
        src_feat = self.feature_transforms(source)
        src_feat = torch.from_numpy(src_feat).float()
    ...
    def collater(
            self, samples: List[AudioToReprDatasetItem], return_order: bool = False
    ) -> Dict:
        """
        src_feat: T x C
        ...
        tgt_lengths = torch.tensor([x.tgt_unit.shape[0] for x in samples], dtype=torch.long)
        ...
        src_feature_size = samples[0].src_feat.shape[1]
        tgt_feature_size = samples[0].tgt_feat.shape[1]

        padded_src = samples[0].src_feat.new_zeros(batch_size, max_src_len, src_feature_size)
        padded_tgt = samples[0].src_feat.new_zeros(batch_size, max_tgt_len, tgt_feature_size)
        ...
        padded_reduce_tgt_feat = samples[0].reduce_tgt_feat.new_zeros(batch_size, max_reduce_tgt_len, tgt_feature_size)
...
class AudioToReprUnitDatasetCreator(object):
    # mandatory columns
    ...
    def _load_samples_from_tsv(src_feat_dir, tgt_feat_dir, raw_audio_root, split):
        ...
        with open(translation_manifest_file) as f:
            ...
            for line in f:
                ...
                # print(src_audio_path, src_n_frames)
                src_audio_path = src_audio_path.replace("/data/pkoehn1/wtan12", "/weka/scratch/jzhan237/diff_s2s")
                ...
                samples.append({
                    AudioToReprUnitDatasetCreator.KEY_ID: src_id,
                    AudioToReprUnitDatasetCreator.KEY_SRC_AUDIO: src_audio_path,
                    AudioToReprUnitDatasetCreator.KEY_SRC_N_FRAMES: src_n_frames,
                    AudioToReprUnitDatasetCreator.KEY_TGT_AUDIO: tgt_feat_path,
                    AudioToReprUnitDatasetCreator.KEY_TGT_UNIT: tgt_tokens,
                    AudioToReprUnitDatasetCreator.KEY_TGT_N_FRAMES: tgt_feat_len,
                })
    ...
    def from_tsv(
        ...
        for split in splits.split(","):
            samples = AudioToReprUnitDatasetCreator._load_samples_from_tsv(
