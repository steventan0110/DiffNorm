...
@register_criterion("repr_to_speech")
class ReprToSpeechLoss(FairseqCriterion):
    ...
    def forward(self, model: FairseqEncoderModel, sample, reduction="mean", save_audio=False, is_training_disc=False, is_training=False):
        ...
        tgt_audio = sample["target"]
        # print("batch size", src_tokens.shape[0])
        waveform, loss_out = model(
            src_tokens,
            src_lengths=None,
            wave_true=tgt_audio,
        )
        ...
        if is_training:
            if is_training_disc:
                ...
                loss = disc_loss + loss_mel*45
            else:
                loss = gen_loss + loss_mel*45
        else:
            loss = gen_loss + loss_mel*45 + disc_loss
            ...
            with torch.no_grad():
                ...
                try:
                    si_snr_score = si_snr(waveform, tgt_audio).item()
                    pesq_score = pesq(waveform, tgt_audio).item()
                except:
