...
@register_criterion("ddpm_loss")
class DDPMLoss(FairseqCriterion):
    def __init__(self, task):
        super().__init__(task)
        self.loss_fn = torch.nn.MSELoss(reduction="mean")
        self.eps = 0.2 # TODO: make this configurable
        self.diff_params = task.diff_params
        self.mu = task.mu
        self.ddpm = create_diffusion(timestep_respacing="")
        print(f"using ddpm diffuser, wit T={self.ddpm.num_timesteps}")
    ...
    def inject_noise(self, target_feat):
        B, Z, L, H = target_feat.shape
        T = self.diff_params["T"]
        alpha_bar = self.diff_params["Alpha_bar"].to(target_feat.device)
        diffusion_steps = torch.randint(T, size=(B, 1, 1, 1)).to(target_feat.device)  # randomly sample diffusion steps from 1~T
        z = torch.normal(0, 1, size=target_feat.shape).to(target_feat.device) # B x Z x L x H
        # z = torch.zeros(B, Z, L, H).to(target_feat.device) # only for debug!!
        transformed_X = torch.sqrt(alpha_bar[diffusion_steps]) * target_feat + torch.sqrt(1 - alpha_bar[diffusion_steps]) * z
        return transformed_X, diffusion_steps.view(B,), z

    def forward(self, model: FairseqEncoderModel, sample, reduction="mean"):
        src_tokens = sample["net_input"]["src_tokens"]
        src_lengths = sample["net_input"]["src_lengths"]
        tgt_feats = sample["target"] # B, 16, T, H
        tgt_lengths = sample["target_lengths"]
        # print("tgt lengths:", tgt_lengths)
        # B, 8, T, H
        tgt_sample = DiagonalGaussianDistribution(tgt_feats).sample()
        scale = 1 / self.mu # computed std from dev set
        tgt_sample = tgt_sample * scale
        tgt_mask = lengths_to_mask(tgt_lengths)

        # ----------------------- DDPM loss computation ----------------------- #
        timesteps = torch.randint(0, self.ddpm.num_timesteps, (tgt_sample.shape[0],), device=tgt_sample.device)
        model_kwargs = dict(src_feature=src_tokens, src_lengths=src_lengths)
        loss_dict = self.ddpm.training_losses(model, tgt_sample, timesteps, model_kwargs)
