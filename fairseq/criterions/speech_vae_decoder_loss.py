...
@register_criterion("speech_vae_decoder_loss")
class SpeechVAEDecoderLoss(FairseqCriterion):
    def __init__(self, task):
        super().__init__(task)
        self.eps = 0.1 # TODO: make this configurable
    ...
    def forward(self, model: FairseqEncoderModel, sample, reduction="mean"):
        ...
        reduce_tgt_feature = sample["reduce_target"]
        reduce_tgt_unit = sample["reduce_target_unit"]
        reduce_tgt_lengths = sample["reduce_target_lengths"]

        # ----------------------- DDPM loss computation ----------------------- #
        model_kwargs = dict(
            src_feature=src_tokens,
            src_lengths=src_lengths,
            tgt_lengths=reduce_tgt_lengths,
            unk_token=self.task.tgt_dict.unk_index,
        )

        mse_loss, lm_pred, kl_loss = model(reduce_tgt_feature, reduce_tgt_unit, **model_kwargs)

        lprobs = model.get_normalized_probs([lm_pred], log_probs=True)
        lprobs = lprobs.view(-1, lprobs.size(-1))
        # compute acc
        target = reduce_tgt_unit.view(-1)
        ...
        loss = 0.1 * loss + 10 * mse_loss + 0.0001 * kl_loss
