...
class Trainer(object):
    ...
    def _build_optimizer(self):
        ...
        from fairseq.optim.composite import CompositeLRScheduler
        if hasattr(self._optimizer, "lr_scheduler") and isinstance(self._optimizer.lr_scheduler, CompositeLRScheduler):
            self._lr_scheduler = self._optimizer.lr_scheduler
        else:
    ...
    def valid_step(self, sample, raise_oom=False, save_example=False):
        ...
        with torch.no_grad():
            ...
            try:
                ...
                # # TODO: hacky code to log a batch of generated audio
                # if save_example and hasattr(self.task.args, "save_audio") and self.task.args.save_audio and self.is_data_parallel_master:
                #     save_root = self.cfg.checkpoint.save_dir
                #     B, T = logging_output["waveform"].shape
                #     for batch_id in range(B):
                #         save_path = os.path.join(save_root, "audios", f"audio_step_{self._num_updates}_batch_{batch_id}.wav")
                #         os.makedirs(os.path.dirname(save_path), exist_ok=True)
                #         waveform = logging_output["waveform"][batch_id].cpu().numpy().astype(np.float32)
                #         # if "vocoder" in self.cfg.common.wandb_project:
                #         #     wandb.log({"audio": wandb.Audio(waveform, sample_rate=16000)})
                #         sf.write(save_path, waveform, 16000)
                #     print("Batch of generated audio saved to ", save_root)
                # if hasattr(self.task.args, "save_audio") and self.task.args.save_audio:
                #     del logging_output["waveform"]
