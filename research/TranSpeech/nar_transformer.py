...
class TransformerUnitDecoder(TransformerDecoder):
    ...
    def __init__(
        ...
        self.cg_prob = args.cg_prob if hasattr(args, "cg_prob") else 0.0
        self.self_prompting = args.use_sp if hasattr(args, "use_sp") else False
        print("#" * 100)
        print("conditional drop out prob: ", self.cg_prob)
        print("Use self prompting: ", self.self_prompting)
        print("#" * 100)
    ...
    def cg_forward(
        ...
        # all src_feature are masked
        cond_drop_mask = prob_mask_like((batch_size,), 1, src_features.device)
        # print(prev_output_tokens[0, :])

        null_feature = self.embed_tokens(torch.tensor([self.dictionary.bos()], device=src_features.device))
        cg_src_features = torch.where(
        ...
        cg_src_mask = torch.where(
            cond_drop_mask[:, None],
            torch.zeros_like(src_mask),
            src_mask,
        )
        encoder_out["encoder_out"][0] = cg_src_features.transpose(0, 1)
        encoder_out["encoder_padding_mask"][0] = cg_src_mask
    ...
    def sp_forward(
        ...
        """
        follow: https://arxiv.org/abs/2305.04044
        """

        # we first generate y_0
        with torch.no_grad():
            x, extra = self.extract_features(
                prev_output_tokens,
                encoder_out=encoder_out,
            ...
            if not features_only:
                ...
                if self.out_proj_n_frames:
                    x = self.out_proj_n_frames(x)
                x = self.output_layer(
                    x.view(bsz, seq_len, self.n_frames_per_step, d))  # [B, T, 512] -> [B, T, dictionary_size]
                x = x.view(bsz, seq_len * self.n_frames_per_step, -1)
                if (
                        incremental_state is None and self.n_frames_per_step > 1
                ):  # teacher-forcing mode in training
            ...
            # do not allow generation of padding/eos/bos/masks
            x[:, :, 0] = -torch.finfo(x.dtype).max
            x[:, :, 1] = -torch.finfo(x.dtype).max
            x[:, :, 2] = -torch.finfo(x.dtype).max
            x[:, :, 3] = -torch.finfo(x.dtype).max
            _, pred_y_0 = torch.max(x, -1)
            pad_mask = prev_output_tokens.eq(self.padding_idx) | prev_output_tokens.eq(2) # hard eos token
            y_0 = prev_output_tokens.clone()
            y_0[~pad_mask] = pred_y_0[~pad_mask]

            # now prefix src feature with y_0; need to take care of the padding
            src_feature = encoder_out["encoder_out"][0].transpose(0, 1)
            src_mask = encoder_out["encoder_padding_mask"][0]
            feature_dim = src_feature.shape[-1]

            src_audio_lengths = torch.sum(~src_mask, -1)
            y_0_lengths = torch.sum(~y_0.eq(self.padding_idx), -1)

            # updated total lengths for each instance
            new_lengths = src_audio_lengths + y_0_lengths
            max_length = new_lengths.max()
            # +1 for advance indexing
            new_feature = src_feature.new_full((src_feature.shape[0], max_length+1, feature_dim), 0.0)
            # B x max_audio_len
            src_audio_index = torch.arange(src_audio_lengths.max(), device=src_feature.device).unsqueeze(0).expand(src_feature.shape[0], -1)
            src_audio_index[src_mask] = max_length
            # to be consistent with feature dim: B x src_audio_len x D
            src_audio_index = src_audio_index.unsqueeze(2).expand(-1, -1, feature_dim)
            new_feature = torch.scatter(new_feature, 1, src_audio_index, src_feature)


            y_0_index = torch.arange(y_0_lengths.max(), device=src_feature.device).unsqueeze(0).expand(src_feature.shape[0], -1)
            y_0_index = y_0_index + src_audio_lengths.unsqueeze(1)
            y_0_index[y_0.eq(self.padding_idx)] = max_length
            y_0_index = y_0_index.unsqueeze(2).expand(-1, -1, feature_dim)

            # B x tgt_len x D
            y_0_feature = self.embed_tokens(y_0)
            # exit(0)
            new_feature = torch.scatter(new_feature, 1, y_0_index, y_0_feature)

            updated_src_feature = new_feature[:, :max_length, :].transpose(0, 1)
            updated_src_mask = lengths_to_padding_mask(new_lengths)

        # now we train the model with the updated conditioning
        encoder_out["encoder_out"][0] = updated_src_feature
        encoder_out["encoder_padding_mask"][0] = updated_src_mask
        x, extra = self.extract_features(
            prev_output_tokens,
            encoder_out=encoder_out,
            incremental_state=incremental_state,
    ...
    def forward(
        ...
        """
        # x: B, tgt_len ,256
        if self.self_prompting and not inference_mode:
            # with 50% chance, we decode with the current model and concate the output with the src feature
            if random.Random().random() > 0.5:
                return self.sp_forward(
                    prev_output_tokens,
                    encoder_out=encoder_out,
                    incremental_state=incremental_state,
        ...
        if self.cg_prob > 0 and not inference_mode:
            # print("using cg forward")
            src_features = encoder_out["encoder_out"][0].transpose(0,1)
            src_mask = encoder_out["encoder_padding_mask"][0]
            batch_size = src_features.shape[0]
            cond_drop_mask = prob_mask_like((batch_size,), self.cg_prob, src_features.device)
            # print(prev_output_tokens[0, :])
...
class NARS2UTTransformerModel(S2STransformerMultitaskModelBase):
    ...
    def add_args(parser):
        parser.add_argument(
            "--use_sp",
            action="store_true",
            help="use self-prompting strategy",
        )
        parser.add_argument(
            "--cg_prob",
            type=float,
    ...
    def initialize_output_tokens(self, encoder_out, src_lengths, true_length=None):
        '''
        ...
        # print("init output token:", initial_output_tokens)
...
def s2ut_architecture_fisher(args):
    ...
    args.dropout = getattr(args, "dropout", 0.1)
    args.cg_prob = getattr(args, "cg_prob", 0.0)
